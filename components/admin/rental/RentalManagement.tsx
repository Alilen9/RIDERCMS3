...
type RentalBatteryStatus = 'available' | 'issued' | 'charging' | 'maintenance';
...
const getRentalFleet = async (): Promise<RentalFleetResponse> => {
  const response = await fetch('/admin/rentals/fleet');

  if (!response.ok) {
    throw new Error(`Failed to load rental fleet (${response.status}).`);
  }
      ...
      <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-white">{battery.id}</h1>
            <p className="mt-1 text-sm text-gray-400">Slot {battery.slotId}</p>
          </div>
          <span className="rounded-full bg-indigo-500/10 px-3 py-1 text-sm capitalize text-indigo-300">
            {battery.status}
          </span>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
          <div>
            ...
            <p className="mt-1 text-xl font-semibold text-white">{battery.soc}%</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Last updated</p>
            <p className="mt-1 text-xl font-semibold text-white">
              {battery.lastUpdated ? new Date(battery.lastUpdated).toLocaleString() : 'N/A'}
            </p>
          </div>
        </div>
      </div>
...
const RentalBatteryCard: React.FC<RentalBatteryCardProps> = ({
  ...
  <div className="rounded-2xl border border-gray-800 bg-gray-900 p-5">
    <div className="flex items-start justify-between gap-3">
      <div className="flex items-center gap-3">
        <div className="rounded-xl bg-indigo-500/10 p-3">
          <BatteryFull className="h-5 w-5 text-indigo-400" />
        </div>
        <div>
          <h3 className="font-semibold text-white">{battery.id}</h3>
          <p className="text-sm text-gray-500">Slot {battery.slotId}</p>
        </div>
      </div>
      <span className="rounded-full bg-indigo-500/10 px-2.5 py-1 text-xs capitalize text-indigo-300">
        {battery.status}
      </span>
    </div>
    ...
    <div className="mt-5 flex items-end justify-between">
      <div>
        <p className="text-sm text-gray-500">Charge</p>
        <p className="mt-1 text-2xl font-bold text-white">{battery.soc}%</p>
      </div>
      <button
        type="button"
    ...
    {battery.currentRental && (
      <p className="mt-4 truncate text-sm text-gray-400">
        Rented by {battery.currentRental.renter.name}
      </p>
    )}
  </div>
);
